...
public class CourseDAOImpl implements CourseDAO {
	...
	public void saveNewCourse(Course course) {
		...
		String query = "INSERT INTO course(course_name, teacherCNP, specialization, year, semester) VALUES(?, ?, ?, ?, ?)";
		...
		try {
			...
			p.setInt(4, course.getYear());
			p.setInt(5, course.getSemester());
			p.executeUpdate();
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	@Override
	public void updateCourse(Course selectedCourse, String oldCourseName) {
		String query = "UPDATE course SET course_name=?, specialization=?,teacherCNP=?, year=?, semester=? WHERE course_name=?";
		try {
			PreparedStatement p = conn.prepareStatement(query);
			p.setString(1, selectedCourse.getName());
			p.setString(2, selectedCourse.getSpecialization().getName());
			p.setString(3, selectedCourse.getTeacher().getCnp());
			p.setInt(4, selectedCourse.getYear());
			p.setInt(5, selectedCourse.getSemester());
			p.setString(6, oldCourseName);
			p.executeUpdate();
		} catch (SQLException e) {
			e.printStackTrace();
		}
