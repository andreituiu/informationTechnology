...
public class TeacherDAOImpl implements TeacherDAO {
	...
	public void update(Teacher teacher) {
		String query = "UPDATE teacher SET name=?,surname=?,password=?,?external_email=?,internal_email=? WHERE cnp=?";
		try {
            PreparedStatement p = conn.prepareStatement(query);
            p.setString(1, teacher.getName());
            p.setString(2, teacher.getSurname());
            p.setString(3, teacher.getPassword());
            p.setString(4, teacher.getExternalEmail());
            p.setString(5, teacher.getInternalEmail());
            p.setString(6, teacher.getCnp());
